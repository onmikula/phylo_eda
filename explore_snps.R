...
### SEQUENCES
# The below pipeline assumes inout in the form of list of alignments corresponding to discrete loci with phased alleles (i.e., in diploid organism, every individual is represented at every locus by two sequences). This can be obtained from the appropriate output of ddRAD-loci assembler like ipyrad or Stacks. The functions 'readIpyrad' and 'readStacks' read in such data and produce the list of locus-specific alignments. The loci can be the filtered, e.g., to retain just those with higher than 80% occupancy (=sequencing success across individuals). The functions are included in this github repository, but not the data file (because of its large size). 

#source("func/readIpyrad.R")
#source("func/readStacks.R")
#geo_loci <- readStacks("data/georychus.alleles.fas")
#geo_loci_subset <- geo_loci[indcoverage(geo_loci) >= 0.80]
#save(geo_loci_subset, file="data/geo_loci_subset.R")
